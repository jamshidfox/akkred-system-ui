...
const Status = styled('div')`
  ...
  padding: 3px 12px;
`
const Analysis = ({ analysis, idAp }) => {
  // Command
  const tableCommandList = analysis.map(client => {
    const {
      id,
      status,
      ...
      >
        <td colSpan={12} style={{
          color: '#0f22ff'
            ...
            color: '#0f22ff'
          }} href={`${API_URL}/main/applications/${idAp}/analysis`}>Hujjat</a></td>
  ...
  return (
    ...
    <>
      <PageRowTitle name="Buyurtmani va rеsurslarni tahlil qilish" />
      {tableCommand}
    </>
