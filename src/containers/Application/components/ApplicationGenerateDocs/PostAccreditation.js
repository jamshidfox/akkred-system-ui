...
const Status = styled('div')`
  ...
  padding: 3px 12px;
`
const PostAccreditation = ({ postAccreditation, idAp }) => {
  const tableCommandList = postAccreditation.map(client => {
    const {
      id,
      status,
      ...
      >
        <td colSpan={12} style={{
          color: '#0f22ff'
            ...
            color: '#0f22ff'
          }} href={`${API_URL}/main/applications/${idAp}/post_accred`}>Hujjat</a></td>
  ...
  const tableCommand =
    <Table
      isEmpty={isEmpty(postAccreditation)}
    >
      {tableCommandHead}
      {tableCommandList}
    </Table>
...
PostAccreditation.defaultProps = {
  postAccreditation: [],
  idAp:1,
}

export default PostAccreditation
