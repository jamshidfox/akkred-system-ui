...
const Status = styled('div')`
  ...
  padding: 3px 12px;
`
const ConclusionGroup = ({ conclusions, idAp }) => {
  // Command
  const tableCommandList = conclusions.map(client => {
    const {
      id,
      status,
      ...
      >
        <td colSpan={12} style={{
          color: '#0f22ff'
            ...
            color: '#0f22ff'
          }} href={`${API_URL}/main/applications/${idAp}/conclusion_group`}>Hujjat</a></td>
  ...
  return (
    ...
    <>
      <PageRowTitle name="Baholash guruhini kelishish" />
      {tableCommand}
    </>

  )
}
ConclusionGroup.defaultProps = {
  conclusions: [],
  idAp:1,
}
