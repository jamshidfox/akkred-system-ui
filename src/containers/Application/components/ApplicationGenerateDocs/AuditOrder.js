...
const Status = styled('div')`
  ...
  padding: 3px 12px;
`
const AuditOrder = ({ auditOrders, idAp }) => {
  // Command
  const tableCommandList = auditOrders.map(client => {
    const {
      id,
      status,
      ...
      >
        <td colSpan={12} style={{
          color: '#0f22ff'
            ...
            color: '#0f22ff'
          }} href={`${API_URL}/main/applications/${idAp}/order`}>Hujjat</a></td>
