...
import ReservationListFilterForm from './ReservationListFilterForm'
...
const PlacingList = props => {
  ...
  const tableActions = (
    <TableActions
      filterForm={<ReservationListFilterForm />}
          ...
          {data.map(placing => {
            ...
            return (
              <TableRow gutter={10} key={id}>
                ...
                <TableCol span={1}>
                  <Link style={style} to={sprintf(PLACING_UPDATE_URL, placing.id)}><img src={Edit}
                    alt="Edit" /></Link>
                </TableCol>
                <TableCol span={1}>
